...
export default function MessagingPage() {
    ...
    const handleSendMessage = async () => {
        ...
        const otherParticipantId = currentConversation?.participants.find(p => p !== userProfile.uid);
        
        if(!otherParticipantId) {
            // This handles the case for a brand new conversation not yet in the state
            ...
            if (!otherId) return; // Should not happen

            const msgRef = collection(firestore, 'conversations', selectedConversationId, 'messages');
            await addDoc(msgRef, { text: newMessage, senderId: userProfile.uid, createdAt: serverTimestamp() });
            const convRef = doc(firestore, 'conversations', selectedConversationId);
            await updateDoc(convRef, { lastMessage: newMessage, lastUpdatedAt: serverTimestamp(), [`unreadBy.${otherId}`]: true });
        } else {
            const msgRef = collection(firestore, 'conversations', selectedConversationId, 'messages');
            await addDoc(msgRef, { text: newMessage, senderId: userProfile.uid, createdAt: serverTimestamp() });
            const convRef = doc(firestore, 'conversations', selectedConversationId);
